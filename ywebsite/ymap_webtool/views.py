...
from .cleanup_thread import *


wd = os.getcwd()
data_path = os.path.join(wd, 'ymap_webtool', 'data')
...
'''
def get_work_folders(user_id):
    input_path = os.path.join(data_path, 'input_'+str(user_id))
    output_path = os.path.join(data_path, 'output_'+str(user_id))
    archive_path = os.path.join(data_path, 'archive_'+str(user_id))
...
>>>>>>> d3997c46448e5414bd8f0a88bb2a15d6dbe6784d
...
def finished(request):
    ...
    logger.debug(
        "time to give back the finished page with the script for downloading the results")
    is_running = False
...
=======
    ...
    '''
    logger.debug("started processing the job")
    logger.debug(request.GET.get("user_id"))
    user_id = str(request.GET.get("user_id"))

    input_path, output_path, archive_path = get_work_folders(user_id)
...
def submitted(request):
    ...
    if request.method == 'POST':
        ...
        # check if the extention of the file is .txt (ps: this is already checked on the html)
        if validate_file_type(request.FILES['myfile'].name) is True:
            logger.debug(request.POST.get("user_id"))
            user_id = request.POST.get("user_id")
            input_path, output_path, archive_path = get_work_folders(user_id)
            save_file(request.FILES['myfile'], input_path, input_file_name)
            logger.debug("the job should be submitted and passed to processing")

            return render(request, 'ymap_webtool/submitted.html', context ={"user_id": user_id})
    else:
        return HttpResponseRedirect('submission_fail')
...
def download_result(request):
    ...
    # the rb flag is needed on windows!motherwise r should be sufficient
    logger.debug(request.GET.get("user_id"))
    user_id = request.GET.get("user_id")
    i,o, archive_path = get_work_folders(user_id)
    resp_file = open(os.path.join(archive_path, archive_name + ".zip"), 'rb')
    response = HttpResponse(
        resp_file, content_type='application/force-download')
    response['Content-Disposition'] = 'attachment; filename="%s"' % download_name
    logger.debug("download the zip file")

    lady = CleaningLady(i,o, archive_path, seconds_to_wait=CleaningLady.HOUR_IN_SECONDS, logger=logger)
    lady.start()
